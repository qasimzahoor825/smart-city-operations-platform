...
const hoursFrom = (h) => new Date(now.getTime() + h * 3_600_000).toISOString();
const hash = (pw) => bcryptjs_1.default.hashSync(pw, 10);
const CITY = {
    center: { lat: 31.52, lon: 74.35 },
    spread: 0.06,
};
function jitter(seedValue) {
...
async function insertIfEmpty(collectionName, docs) {
    const rows = docs.filter((d) => d !== null);
    if (rows.length === 0)
        return 0;
...
function buildEmergencies(count) {
    const types = [
...
function buildAppointments(count) {
...
function buildServiceRequests(count) {
    const services = reference_1.seedServices;
...
function buildFeedback(resolvedComplaints) {
    const ratings = [5, 5, 4, 4, 3, 5, 4, 5];
    const out = [];
    resolvedComplaints
        .filter((c) => c !== null && ["RESOLVED", "CITIZEN_FEEDBACK", "CLOSED"].includes(String(c.status)))
        .slice(0, 8)
        .forEach((c, i) => {
        out.push({
            id: `fb_seed_${i + 1}`,
...
async function main() {
    ...
    // Reference data
    let n = await upsertAll("sla_rules", reference_1.seedSlaRules.map((r) => ({ ...r })));
    console.log(`  sla_rules: ${n}`);
    n = await upsertAll("complaint_categories", reference_1.seedComplaintCategories.map((c) => ({ ...c })));
    console.log(`  complaint_categories: ${n}`);
    n = await upsertAll("services", reference_1.seedServices.map((s) => ({ ...s })));
    console.log(`  services: ${n}`);
    n = await upsertAll("traffic_zones", reference_1.seedTrafficZones.map((z) => ({ ...z })));
    console.log(`  traffic_zones: ${n}`);
    n = await upsertAll("announcements", reference_1.seedAnnouncements.map((a) => ({ ...a, publishedAt: a.publishedAt ?? new Date().toISOString() })));
    ...
    n = await upsertAll("users", extraCitizens.map((c) => ({
        id: c.id,
        fullName: c.fullName,
        email: c.email,
